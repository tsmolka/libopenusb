	...
	unsigned char index, void *buf, unsigned int buflen)
{
	...
	return libusb_ctrl_xfer(dev,0,0,&ctrl);
}
	...
	unsigned char *buf, unsigned int buflen)
{
	...
	while (buflen >= USBI_INTERFACE_DESC_SIZE) {
		int altlen = sizeof(intf->altsettings[0]) 
			* (intf->num_altsettings+1);
		libusb_parse_data("bb", buf, buflen, &header,
				sizeof(header), &count);

		intf->altsettings = realloc(intf->altsettings,altlen);

		if (!intf->altsettings) {
			intf->num_altsettings = 0;
			usbi_debug(NULL, 1, "couldn't allocated %d"
				" bytes for altsettings", altlen);
		...
		usbi_debug(NULL, 4, "interface: num = %d, alt = %d",
		...
		if (as->desc.bNumEndpoints > USBI_MAXENDPOINTS) {
			usbi_debug(NULL, 1, "too many endpoints, ignoring rest");
			free(intf->altsettings);
			return -1;
		}
		...
		memset(as->endpoints, 0,
				as->num_endpoints * sizeof(as->endpoints[0]));
	...
	size_t buflen)
{
	...
	usbi_debug(NULL, 4, "parse cfg desc: %d", count);
	...
	cfg->interfaces = malloc(cfg->desc.bNumInterfaces *
	...
	for (i = 0; (i < cfg->num_interfaces) && (buflen > 0); i++) {
		...
		retval = usbi_parse_interface(cfg->interfaces + i, buf,
				buflen);
		if (retval < 0) {
			free(cfg->interfaces);
			return retval;
...
int usbi_fetch_and_parse_descriptors(struct usbi_dev_handle *hdev)
{
  ...
  dev->desc.configs_raw = malloc(dev->desc.num_configs *
		  sizeof(dev->desc.configs_raw[0]));
  if (!dev->desc.configs_raw) {
    usbi_debug(NULL, 1, "unable to allocate %d bytes for cached descriptors",
	dev->desc.num_configs * sizeof(dev->desc.configs_raw[0]));
    goto err;
  }

  memset(dev->desc.configs_raw, 0, dev->desc.num_configs *
		  sizeof(dev->desc.configs_raw[0]));

  dev->desc.configs = malloc(dev->desc.num_configs * 
		  sizeof(dev->desc.configs[0]));
  if (!dev->desc.configs) {
    usbi_debug(NULL, 1, "unable to allocate memory for config descriptors",
	dev->desc.num_configs * sizeof(dev->desc.configs[0]));
    goto err;
  }

  memset(dev->desc.configs, 0, dev->desc.num_configs * 
		  sizeof(dev->desc.configs[0]));
  ...
  for (i = 0; i < dev->desc.num_configs; i++) {
    ...
    ret = usbi_get_descriptor(hdev->handle, USB_DESC_TYPE_CONFIG, i, buf, 8);
    if (ret < 8) {
      if (ret < 0)
        usbi_debug(NULL, 1, "unable to get first 8 bytes of config"
		" descriptor (ret = %d)", ret);
      else
        usbi_debug(NULL,1, "config descriptor too short (expected 8, got %d)",
		ret);
    ...
    if (!cfgr->data) {
      usbi_debug(NULL, 1, "unable to allocate %d bytes for descriptors",
		      cfgr->len);
      goto err;
    }
    ...
    if (ret < cfgr->len) {
      if (ret < 0)
        usbi_debug(NULL, 1, "unable to get rest of config descriptor"
		" (ret = %d)", ret);
      else
    ...
    ret = usbi_parse_configuration(dev->desc.configs + i, cfgr->data,
		    cfgr->len);
    if (ret > 0)
